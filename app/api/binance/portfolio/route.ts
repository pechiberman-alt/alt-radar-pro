...
export async function GET(request: Request) {
  try {
    ...
    const [balances, deposits, withdrawals] = await Promise.all([
      ...
      getDepositHistory(apiKey, apiSecret).catch(() => []),
      getWithdrawHistory(apiKey, apiSecret).catch(() => []),
    ]);

    return Response.json(
      { balances: balances.balances, updateTime: balances.updateTime, deposits, withdrawals },
